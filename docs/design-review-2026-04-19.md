...
- Permission guide зробити як окремий shared компонент (`PermissionGate`), бо і Clipboard (Accessibility), і Recorder його потребують.
...
| **PermissionGate** (request + guide + open-settings) | Accessibility (Clipboard), Screen/Camera/Mic (Recorder) | 🟡 |
...
10. PermissionGate shared компонент (Accessibility + Screen/Camera/Mic). (M)
...
| № | Пункт | Причина |
|---|---|---|
| 10 | `PermissionGate` shared компонент | Recorder-Swift helper ще не реалізований — до нього не існує permission flow, який можна об'єднати з Clipboard Accessibility. Краще витягувати спільний компонент, коли матимемо другий живий приклад, а не спекулятивно. |
