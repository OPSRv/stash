...
### Worked examples (match this decisiveness and EQ shaping)

Djent / modern metal rhythm — tight low end (Bass 28), boosted mids (Middle 72),
TS-808 in front as a LOW-gain tightening boost, gate on, EQ cuts 100Hz, no delay:
{
  "name": "Djent",
  "patchVOL": 50,
  "blocks": {
    "nr":  { "on": true,  "model": 0,  "params": { "THRE": 38 } },
    "pre": { "on": false },
    "dst": { "on": true,  "model": 0,  "params": { "Gain": 20, "Tone": 65, "VOL": 55 } },
    "amp": { "on": true,  "model": 20, "params": { "Gain": 78, "VOL": 50, "Bass": 28, "Middle": 72, "Treble": 56, "PRES": 60 } },
    "cab": { "on": true,  "model": 13, "params": { "VOL": 50 } },
    "eq":  { "on": true,  "model": 1,  "params": { "100Hz": -10, "1kHz": 4, "3kHz": 3, "VOL": 50 } },
    "mod": { "on": false },
    "dly": { "on": false },
    "rvb": { "on": true,  "model": 1,  "params": { "Mix": 12, "Decay": 30, "Trail": 1 } },
    "ns":  { "on": false }
  }
}

Blues lead — TS-808 into a Bassman-style breakup, mids up (Middle 60), spring
reverb + a touch of slapback, no gate:
{
  "name": "Blues",
  "patchVOL": 50,
  "blocks": {
    "nr":  { "on": false },
    "pre": { "on": false },
    "dst": { "on": true,  "model": 0, "params": { "Gain": 40, "Tone": 62, "VOL": 55 } },
    "amp": { "on": true,  "model": 1, "params": { "Gain": 42, "PRES": 55, "VOL": 50, "Bass": 52, "Middle": 60, "Treble": 55 } },
    "cab": { "on": true,  "model": 4, "params": { "VOL": 50 } },
    "eq":  { "on": false },
    "mod": { "on": false },
    "dly": { "on": true,  "model": 2, "params": { "Mix": 38, "Time (ms)": 120, "F.Back": 12, "Trail": 1 } },
    "rvb": { "on": true,  "model": 6, "params": { "Mix": 25, "Decay": 45, "Trail": 1 } },
    "ns":  { "on": false }
  }
}"##;
