...
- **No visible scrollbars, anywhere**: the app has no scrollbars — every scroller scrolls by wheel/trackpad with the bar fully hidden. The global rules in `src/styles/tokens.css` (`::-webkit-scrollbar { display:none; width:0; height:0 }` + `scrollbar-width: none`) hide it app-wide. Do NOT re-introduce a styled/visible bar (no `scrollbar-width: thin`, no coloured `::-webkit-scrollbar-thumb`, no `scrollbar-gutter`) in any module CSS — WKWebView ignores bar *styling* and would fall back to the chunky native bar, but it honours the zero-size *hide*, which is why this works in the shipped `.app`. If a scroller needs a visual affordance, use a fade-edge hint (see `.terminal-tabbar-scroll`), not a bar.
