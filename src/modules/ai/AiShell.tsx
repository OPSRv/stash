...
/// API-chat tab. Embedded web services (Claude/ChatGPT/Gemini/…) moved to
/// their own `web` module — this shell is just sessions + streaming chat.
export const AiShell = () => {
  ...
  // session id, and here we activate it.
  useEffect(() => {
  ...
  // Pre-fill the composer from cross-module triggers (e.g. clipboard's magic
  // wand, the notes transcript "Ask AI" button). Payload is either a plain
  // string (reuse current session) or `{ text, newSession }`.
  useEffect(() => {
  ...
  const emptyHero = (
    <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center">
      ...
      <div className="t-tertiary text-meta max-w-[420px]">
        Chat is streamed from {settings.aiProvider}. Press ⌘N for a new chat,
        Enter to send.
      </div>
    </div>
  );
