...
EOF
  ...
  # Bundle as a passwordless .p12 so `security import` can handle both.
  ...
  # Suppress the "always allow"/"deny" dialog on first codesign call.
  # SecAccessSetMatchingApplications via `set-key-partition-list` is the
  # documented one-shot fix; it asks once for the keychain password and
  # then never again.
  log "Granting codesign partition access (you may be prompted for your"
  log "macOS login password once — that's the keychain unlock)…"
...
# ── 5. Patch tauri.conf.json (only if signingIdentity is empty/missing) ────
if [[ -f "$CONF" ]]; then
  CURRENT="$(/usr/bin/python3 -c "
import json, sys
with open('$CONF') as f: cfg = json.load(f)
print((cfg.get('bundle', {}).get('macOS', {}) or {}).get('signingIdentity') or '')
")"

  if [[ -z "$CURRENT" ]]; then
    log "Setting bundle.macOS.signingIdentity = \"$CN\""
    /usr/bin/python3 - <<PY
import json, pathlib
p = pathlib.Path("$CONF")
cfg = json.loads(p.read_text())
cfg.setdefault("bundle", {}).setdefault("macOS", {})["signingIdentity"] = "$CN"
p.write_text(json.dumps(cfg, indent=2) + "\n")
PY
    ok "Patched $CONF"
  else
    warn "bundle.macOS.signingIdentity already set to \"$CURRENT\" — leaving alone."
    warn "If you want self-signed instead, clear that field manually and rerun."
  fi
else
